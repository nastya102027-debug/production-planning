...
export function PlanningScreen({onOpenCenter,onOpenTemplates}:{onOpenCenter:(id:string)=>void;onOpenTemplates:()=>void}){
  ...
  return <div className="content planning-page">
    ...
    <section className="planning-workspace">
      ...
      <main className="launch-editor">{!selectedOrder?<div className="planning-empty"><Boxes/><b>Нет заказов для планирования</b></div>:<>
        ...
        <div className="launch-items"><div className="launch-items-title"><b>Позиции запуска</b><span>Можно выбрать часть заказа или часть количества</span></div>{selectedOrder.items.map(item=>{
          const launched=launchedByItem[item.id]||0;const available=Math.max(0,item.quantity-launched);const draft=drafts[item.id]||{selected:false,quantity:"",routeId:""};const itemRoutes=routes[item.id]||[];const pickerNeedle=(routeQueries[item.id]??"").trim().toLocaleLowerCase("ru");const matchingTemplates=templates.filter(template=>!pickerNeedle||[template.name,template.description??"",template.category??""].join(" ").toLocaleLowerCase("ru").includes(pickerNeedle));const matchingRoutes=itemRoutes.filter(route=>route.id===draft.routeId||!pickerNeedle||route.name.toLocaleLowerCase("ru").includes(pickerNeedle));const selectedRoute=itemRoutes.find(route=>route.id===draft.routeId);
          return <article className={`${draft.selected?"selected":""} ${available===0?"disabled":""}`} key={item.id}><label className="item-check"><input type="checkbox" checked={draft.selected} disabled={available===0} onChange={event=>patchDraft(item.id,{selected:event.target.checked})}/><span/></label><div className="item-name"><b>{item.name}</b><span>Заказано: {item.quantity} · запущено: {launched} · доступно: {available}</span></div><label>Количество<input type="number" min="1" max={available} value={draft.quantity} disabled={!draft.selected} onChange={event=>patchDraft(item.id,{quantity:event.target.value?Number(event.target.value):""})}/></label><div className="route-choice"><span>Маршрут или шаблон</span><button type="button" className={`route-choice-toggle ${selectedRoute?"selected":""}`} aria-expanded={routePicker===item.id} onClick={()=>setRoutePicker(current=>current===item.id?"":item.id)} disabled={available===0}><b>{selectedRoute?`${selectedRoute.name} · ${selectedRoute.steps.length} эт.`:"Выберите маршрут или шаблон"}</b><ArrowDown size={16}/></button>{routePicker===item.id&&<div className="route-choice-menu"><input aria-label={`Найти маршрут или шаблон для ${item.name}`} autoFocus placeholder="Найти шаблон или маршрут" value={routeQueries[item.id]??""} onChange={event=>setRouteQueries(current=>({...current,[item.id]:event.target.value}))}/><div className="route-choice-results">{matchingTemplates.length>0&&<section><small>Шаблоны</small>{matchingTemplates.map(template=><button type="button" className="route-choice-result" key={template.id} onClick={()=>void applyTemplate(item,template.id)}><span><b>{template.name}</b>{template.category&&<small>{template.category}</small>}</span><em>Шаблон</em></button>)}</section>}{matchingRoutes.length>0&&<section><small>Сохранённые маршруты</small>{matchingRoutes.map(route=><button type="button" className="route-choice-result" key={route.id} onClick={()=>{patchDraft(item.id,{selected:true,quantity:drafts[item.id]?.quantity||1,routeId:route.id});setRoutePicker("");}}><span><b>{route.name}</b><small>{route.steps.length} эт.</small></span><em>Маршрут</em></button>)}</section>}{matchingTemplates.length===0&&matchingRoutes.length===0&&<p>Ничего не найдено</p>}</div><button type="button" className="route-library-link" onClick={onOpenTemplates}>Открыть библиотеку шаблонов</button></div>}</div><button className="route-button" onClick={()=>{setEditRoute(undefined);setRouteItem(item);}}><RouteIcon/>{itemRoutes.length?"Новый":"Создать"}</button>{selectedRoute&&<button className="route-button" onClick={()=>{setEditRoute(selectedRoute);setRouteItem(item);}}>Изменить</button>}{selectedRoute&&<button className="route-button route-delete" onClick={()=>void deleteRoute(item,selectedRoute)}><Trash2/>Удалить</button>}</article>;
        })}</div>
    ...
    {routeItem&&<Suspense fallback={<div className="modal-backdrop"><div className="task-dialog"><p>Открываю конструктор маршрута…</p></div></div>}><RouteBuilder initial={editRoute} item={routeItem} centers={centers} onClose={()=>setRouteItem(null)} onSaved={route=>{setRoutes(current=>({...current,[routeItem.id]:[route,...(current[routeItem.id]||[])]}));patchDraft(routeItem.id,{routeId:route.id});setRouteItem(null);}}/></Suspense>}
  </div>;
}
