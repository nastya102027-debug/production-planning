...
export function PlanningScreen({onOpenCenter}:{onOpenCenter:(id:string)=>void}){
  ...
  const routeSearchNeedle=routeSearch.trim().toLocaleLowerCase("ru");
  ...
  return <div className="content planning-page">
    ...
    <section className="planning-workspace">
      ...
      <main className="launch-editor">{!selectedOrder?<div className="planning-empty"><Boxes/><b>Нет заказов для планирования</b></div>:<>
        ...
        <div className="launch-items"><div className="launch-items-title"><b>Позиции запуска</b><span>Можно выбрать часть заказа или часть количества</span></div><label className="route-search">Найти маршрут или шаблон<input aria-label="Найти маршрут или шаблон" placeholder="Введите название" value={routeSearch} onChange={event=>setRouteSearch(event.target.value)}/></label>{selectedOrder.items.map(item=>{
          const launched=launchedByItem[item.id]||0;const available=Math.max(0,item.quantity-launched);const draft=drafts[item.id]||{selected:false,quantity:"",routeId:""};const itemRoutes=routes[item.id]||[];const matchingTemplates=templates.filter(template=>!routeSearchNeedle||[template.name,template.description??"",template.category??""].join(" ").toLocaleLowerCase("ru").includes(routeSearchNeedle));const matchingRoutes=itemRoutes.filter(route=>route.id===draft.routeId||!routeSearchNeedle||route.name.toLocaleLowerCase("ru").includes(routeSearchNeedle));const selectedRoute=itemRoutes.find(route=>route.id===draft.routeId);
          return <article className={`${draft.selected?"selected":""} ${available===0?"disabled":""}`} key={item.id}><label className="item-check"><input type="checkbox" checked={draft.selected} disabled={available===0} onChange={event=>patchDraft(item.id,{selected:event.target.checked})}/><span/></label><div className="item-name"><b>{item.name}</b><span>Заказано: {item.quantity} · запущено: {launched} · доступно: {available}</span></div><label>Количество<input type="number" min="1" max={available} value={draft.quantity} disabled={!draft.selected} onChange={event=>patchDraft(item.id,{quantity:event.target.value?Number(event.target.value):""})}/></label><label className="route-choice">Маршрут или шаблон<select aria-label={`Маршрут или шаблон для ${item.name}`} value={draft.routeId?"route:"+draft.routeId:""} disabled={available===0} onChange={event=>{const [kind,id]=event.target.value.split(":");if(kind==="template")void applyTemplate(item,id);else if(kind==="route")patchDraft(item.id,{selected:true,quantity:drafts[item.id]?.quantity||1,routeId:id});}}><option value="">Выберите маршрут или шаблон</option>{matchingTemplates.length>0&&<optgroup label="Шаблоны">{matchingTemplates.map(template=><option value={"template:"+template.id} key={template.id}>{template.name} · {template.steps.length} эт.</option>)}</optgroup>}{matchingRoutes.length>0&&<optgroup label="Сохранённые маршруты">{matchingRoutes.map(route=><option value={"route:"+route.id} key={route.id}>{route.name} · {route.steps.length} эт.</option>)}</optgroup>}</select></label><button className="route-button" onClick={()=>{setEditRoute(undefined);setRouteItem(item);}}><RouteIcon/>{itemRoutes.length?"Новый":"Создать"}</button>{selectedRoute&&<button className="route-button" onClick={()=>{setEditRoute(selectedRoute);setRouteItem(item);}}>Изменить</button>}{selectedRoute&&<button className="route-button route-delete" onClick={()=>void deleteRoute(item,selectedRoute)}><Trash2/>Удалить</button>}</article>;
        })}</div>
