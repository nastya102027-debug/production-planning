...
function GlobalSearch({onSelect}:{onSelect:(item:SearchResult)=>void}){
 ...
 return <div className="global-search"><Search size={16}/><input aria-label="Глобальный поиск" placeholder="Поиск" value={q} onFocus={()=>setOpen(true)} onChange={e=>{setQ(e.target.value);setOpen(true);}}/>{open&&q.trim().length>=2&&<div className="global-results">{items.length?items.map(item=><button key={`${item.type}-${item.id}`} onMouseDown={e=>e.preventDefault()} onClick={()=>{onSelect(item);setQ("");setOpen(false);}}><b>{item.title}</b><small>{item.detail}</small></button>):<span>Ничего не найдено</span>}</div>}</div>;
}
