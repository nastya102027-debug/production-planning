...
<span>{order.items.length}</span><span>{money(order.completedTotal)}</span><span><b>{money(order.total)}</b>{finance[order.id]?.income&&<small className="finance-line income">К выставлению: {money(finance[order.id].income!.total)}<em>{finance[order.id].income!.centers.map(row=>`${row.workCenter} — ${money(row.amount)}`).join(" · ")}</em></small>}{finance[order.id]?.contractor.total>0&&<small className="finance-line expense">Подрядчикам: {money(finance[order.id].contractor.total)}<em>{finance[order.id].contractor.centers.map(row=>`${row.workCenter} — ${money(row.amount)}`).join(" · ")}</em></small>}{finance[order.id]?.missingRates.length>0&&<small className="finance-missing">Нет тарифа: {finance[order.id].missingRates.join(", ")}</small>}{!archived&&<button className="secondary" onClick={()=>{setEditing(order);setShowForm(true);}}>{viewOnly?"Открыть":"Изменить"}</button>}{!archived&&<button className="secondary" onClick={()=>setForecastOrder(order)}>Прогноз</button>}<button className="secondary" onClick={()=>setFilesOrder(order)}>Файлы</button><button className="secondary" onClick={()=>onOpenChat(order.id)}>Чат</button><button data-edit className="secondary" disabled={clearing} onClick={()=>void archive(order)}>{archived?"Восстановить":"В архив"}</button>{archived&&<button data-edit className="secondary" disabled={clearing} onClick={()=>void clearArchive(order)}>Очистить</button>}</span>
      </div>)}
    </div>
