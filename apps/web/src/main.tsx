...
type Page = "overview"|"orders"|"procurement"|"launches"|"problems";
...
function PlannerOverview() {
  ...
  useEffect(()=>{api<Summary>("/planner/summary").then(setSummary);},[]);
  return <div className="content"><section className="metrics"><Metric icon={ClipboardList} value={summary?.orders??0} label="активных заказов"/><Metric icon={PackageCheck} value={summary?.inProcurement??0} label="в закупке"/><Metric icon={Factory} value={summary?.operations??0} label="операций в работе"/><Metric icon={AlertTriangle} value={summary?.stopped??0} label="остановлено" tone="danger"/></section><section className="grid"><EmptyPanel eyebrow="ТРЕБУЕТ ВНИМАНИЯ" title="Проблемы и горящие сроки" icon={AlertTriangle} text="Нет активных проблем"/><EmptyPanel eyebrow="ПЛАН НА СЕГОДНЯ" title="Что нужно запустить" icon={Boxes} text="Запуски не запланированы"/></section></div>;
}
...
function EmployeeScreen() { return <div className="content"><section className="employee-status"><div><small>СЕЙЧАС В РАБОТЕ</small><h2>Нет активной задачи</h2><p>Начните следующую задачу из очереди вашего участка.</p></div><button disabled>Завершить текущую</button></section><EmptyPanel eyebrow="ОЧЕРЕДЬ УЧАСТКА" title="Мои задачи" icon={ClipboardList} text="Очередь пуста"/></div>; }

function Shell({user,onLogout}:{user:User;onLogout:()=>void}) {
  const employee=user.role==="EMPLOYEE"; const [page,setPage]=useState<Page>("overview");
  const titles:Record<Page,string>={overview:employee?`Мой участок — ${user.workCenters[0]?.workCenter.name??"не назначен"}`:"Производство сегодня",orders:"Заказы",procurement:"Закупка",launches:"Производственные запуски",problems:"Проблемы"};
  const nav=(target:Page,Icon:typeof Factory,label:string)=><button className={page===target?"active":""} onClick={()=>setPage(target)}><Icon/> {label}</button>;
  return <div className="shell"><aside><div className="logo"><span>К</span><b>КОНТУР</b></div><nav>{nav("overview",Factory,employee?"Мой участок":"Обзор")}{!employee&&<>{nav("orders",ClipboardList,"Заказы")}{nav("procurement",PackageCheck,"Закупка")}{nav("launches",Boxes,"Запуски")}{nav("problems",AlertTriangle,"Проблемы")}</>}</nav><button className="logout" onClick={onLogout}><LogOut/> Выйти</button></aside><main><header><div><p>{employee?"РАБОЧЕЕ МЕСТО":"ЦЕНТР УПРАВЛЕНИЯ"}</p><h1>{titles[page]}</h1></div><div className="header-tools"><span className="avatar">{user.firstName[0]}{user.lastName[0]}</span><div><b>{user.firstName} {user.lastName}</b><small>{employee?"Сотрудник участка":"Планер"}</small></div></div></header>{employee?<EmployeeScreen/>:page==="orders"?<OrdersScreen/>:page==="procurement"?<ProcurementScreen/>:page==="overview"?<PlannerOverview/>:<div className="content"><EmptyPanel eyebrow="РАЗДЕЛ" title={titles[page]} icon={Factory} text="Раздел готовится"/></div>}</main></div>;
}
