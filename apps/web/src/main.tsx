...
type OrderFinance = {orderId:string;income:{total:number;centers:{workCenter:string;amount:number}[]} | null;contractor:{total:number;centers:{workCenter:string;amount:number}[]};missingRates:string[]};
...
function OrdersScreen({onOpenChat}:{onOpenChat:(orderId:string)=>void}) {
  ...
  const [orders,setOrders]=useState<Order[]>([]); const [finance,setFinance]=useState<Record<string,OrderFinance>>({}); const [loading,setLoading]=useState(true); const [search,setSearch]=useState(""); const [showForm,setShowForm]=useState(false); const [editing,setEditing]=useState<Order>(); const [archived,setArchived]=useState(false); const [error,setError]=useState(""); const [confirmation,setConfirmation]=useState<Confirmation|null>(null);
  const load=async()=>{setLoading(true);try{const next=await api<Order[]>(`/orders?archived=${archived}&search=${encodeURIComponent(search)}`);setOrders(next);const result=next.length?await api<{items:OrderFinance[]}>(`/orders/finance?ids=${next.map(order=>order.id).join(",")}`):{items:[]};setFinance(Object.fromEntries(result.items.map(item=>[item.orderId,item])));setError("");}catch(e){setError(e instanceof Error?e.message:"Не удалось загрузить заказы");}finally{setLoading(false);}};
  ...
  return <div className="content orders-page">
    ...
    <div className="order-table">
      ...
      {loading?<div className="table-empty">Загрузка…</div>:orders.length===0?<div className="table-empty"><ClipboardList/><b>{archived?"Архив пуст":"Заказов пока нет"}</b><span>{archived?"Нет записей по текущему поиску":"Создайте первый заказ вручную"}</span></div>:orders.map(order=><div className="order-row" key={order.id}>
        <span className="order-identity"><b>Производство № {order.productionOrderNumber}</b><small>Заказ покупателя № {order.customerOrderNumber||"—"}</small>{order.organization&&<i className={`organization-badge org-${order.organization.toLowerCase()}`}>{organizationLabel[order.organization]}</i>}<small className="order-nomenclature"><strong>Номенклатура:</strong> {order.items.slice(0,2).map(item=>`${item.name} · ${item.quantity} шт.`).join("; ")}{order.items.length>2?` и ещё ${order.items.length-2}`:""}</small></span>
...
<span>{order.items.length}</span><span>{money(order.completedTotal)}</span><span><b>{money(order.total)}</b></span><div className="order-row-bottom">{(finance[order.id]?.income||finance[order.id]?.contractor.total||finance[order.id]?.missingRates.length)&&<div className="order-finance"><small>ФИНАНСЫ ПО ЗАКАЗУ</small><div>{finance[order.id]?.income&&<section className="finance-card income"><span>К выставлению</span><b>{money(finance[order.id].income!.total)}</b><em>{finance[order.id].income!.centers.map(row=>`${row.workCenter} — ${money(row.amount)}`).join(" · ")}</em></section>}{finance[order.id]?.contractor.total>0&&<section className="finance-card expense"><span>Подрядчикам</span><b>{money(finance[order.id].contractor.total)}</b><em>{finance[order.id].contractor.centers.map(row=>`${row.workCenter} — ${money(row.amount)}`).join(" · ")}</em></section>}{finance[order.id]?.missingRates.length>0&&<section className="finance-card missing"><span>Не заданы тарифы</span><b>{finance[order.id].missingRates.join(", ")}</b></section>}</div></div>}<div className="order-actions">{!archived&&<button className="secondary" onClick={()=>{setEditing(order);setShowForm(true);}}>{viewOnly?"Открыть":"Изменить"}</button>}{!archived&&<button className="secondary" onClick={()=>setForecastOrder(order)}>Прогноз</button>}<button className="secondary" onClick={()=>setFilesOrder(order)}>Файлы</button><button className="secondary" onClick={()=>onOpenChat(order.id)}>Чат</button><button data-edit className="secondary" disabled={clearing} onClick={()=>void archive(order)}>{archived?"Восстановить":"В архив"}</button>{archived&&<button data-edit className="secondary" disabled={clearing} onClick={()=>void clearArchive(order)}>Очистить</button>}</div></div>
      </div>)}
    </div>
...
function Shell({user,onLogout}:{user:User;onLogout:()=>void}) {
  ...
  return <ViewOnly.Provider value={viewOnly}><LiveNotifications planner={!employee}/><div className={`shell ${employee?"employee-shell":"planner-shell"} ${viewOnly?"view-only":""} ${menuOpen?"menu-open":""}`}>
