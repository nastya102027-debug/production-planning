...
type Operation = { id: string; title: string; quantity: number; status: string; priority: string; dueDate?: string; plannedStart?: string; comment?: string; stopReason?: string;
...
function TaskCard({ task, onAction, onOpen, now }: { task: Operation; onAction: (task: Operation, action: string) => void; onOpen: () => void; now: number }) {
  ...
  return <article className={`kanban-task ${task.status.toLowerCase()}`}>
    ...
    <small>Запуск № {task.launchNumber}{task.plannedStart ? ` · начало ${date(task.plannedStart)}` : ""}</small>
