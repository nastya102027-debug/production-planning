...
function PositionCard({group,onOpen,labels}:{group:PositionGroup;onOpen:(task:Operation)=>void;labels:Record<string,string>}) {
  ...
  return <article className={`position-card ${status.toLowerCase()}`}>
...
export function OperationsScreen({ planner = false, initialCenter = "", focusId = "", initialStatus = "" }: { planner?: boolean; initialCenter?: string; focusId?: string; initialStatus?:string }) {
  ...
  useEffect(()=>{api<{code:string;name:string}[]>("/operation-statuses").then(rows=>setLabels(current=>({...current,...Object.fromEntries(rows.map(row=>[row.code,row.name]))}))).catch(()=>{});},[]);
  ...
  return <div className="content production-board">
    ...
    {loading ? <p>Загрузка задач…</p> : view === "timeline" && planner ? <Timeline tasks={data.items} now={now} labels={labels} onOpen={task=>setOpened(task)}/> : <div className="kanban">{Object.entries(labels).map(([key, label]) => {const rows=positions.filter(group=>positionStatus(group.tasks)===key);return <section className={`kanban-column ${key.toLowerCase()}`} key={key}><h3>{label}<span>{rows.length}</span></h3>{rows.map(group=><PositionCard key={group.id} group={group} labels={labels} onOpen={setOpened}/>)}{!rows.length && <p className="column-empty">Нет позиций</p>}</section>;})}</div>}
...
function Timeline({tasks,now,labels,onOpen}:{tasks:Operation[];now:number;labels:Record<string,string>;onOpen:(task:Operation)=>void}) {
  ...
  return <div className="timeline" aria-label="Timeline задач"><div className="timeline-scale"><span>{new Date(min).toLocaleDateString("ru-RU")}</span><span>{new Date(max).toLocaleDateString("ru-RU")}</span></div>{rows.map(row=><button className={`timeline-row ${row.status.toLowerCase()}`} key={row.id} onClick={()=>onOpen(row)}><span className="timeline-label"><b>{row.title}</b><small>{row.orderNumber} · {row.workCenter.name}</small></span><span className="timeline-track"><i style={{left:`${Math.max(0,(row.start-min)/span*100)}%`,width:`${Math.max(2,(row.end-row.start)/span*100)}%`}}/><em>{labels[row.status]||row.status}</em></span></button>)}</div>;
}
