...
const labels: Record<string, string> = { QUEUED: "К запуску", IN_PROGRESS: "В работе", PAUSED: "Остановлено", COMPLETED: "Готово", CANCELLED: "Отменено" };
...
function TaskCard({ task, onAction, onOpen, now, canCancel = false }: { task: Operation; onAction: (task: Operation, action: string) => void; onOpen: () => void; now: number; canCancel?: boolean }) {
...
function PositionCard({group,onOpen}:{group:PositionGroup;onOpen:(task:Operation)=>void}) {
...
export function OperationsScreen({ planner = false, initialCenter = "", focusId = "", initialStatus = "" }: { planner?: boolean; initialCenter?: string; focusId?: string; initialStatus?:string }) {
  ...
  return <div className="content production-board">
    ...
    {loading ? <p>Загрузка задач…</p> : view === "timeline" && planner ? <Timeline tasks={data.items} now={now} onOpen={task=>setOpened(task)}/> : <div className="kanban">{Object.entries(labels).map(([key, label]) => {const rows=positions.filter(group=>positionStatus(group.tasks)===key);return <section className={`kanban-column ${key.toLowerCase()}`} key={key}><h3>{label}<span>{rows.length}</span></h3>{rows.map(group=><PositionCard key={group.id} group={group} onOpen={setOpened}/>)}{!rows.length && <p className="column-empty">Нет позиций</p>}</section>;})}</div>}
...
function Timeline({tasks,now,onOpen}:{tasks:Operation[];now:number;onOpen:(task:Operation)=>void}) {
  ...
  return <div className="timeline" aria-label="Timeline задач"><div className="timeline-scale"><span>{new Date(min).toLocaleDateString("ru-RU")}</span><span>{new Date(max).toLocaleDateString("ru-RU")}</span></div>{rows.map(row=><button className={`timeline-row ${row.status.toLowerCase()}`} key={row.id} onClick={()=>onOpen(row)}><span className="timeline-label"><b>{row.title}</b><small>{row.orderNumber} · {row.workCenter.name}</small></span><span className="timeline-track"><i style={{left:`${Math.max(0,(row.start-min)/span*100)}%`,width:`${Math.max(2,(row.end-row.start)/span*100)}%`}}/><em>{row.status==="PAUSED"?"Остановлено":labels[row.status]}</em></span></button>)}</div>;
}
