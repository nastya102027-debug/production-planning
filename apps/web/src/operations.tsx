...
export function OperationsScreen({ planner = false, initialCenter = "", focusId = "", initialStatus = "" }: { planner?: boolean; initialCenter?: string; focusId?: string; initialStatus?:string }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const query = new URLSearchParams({ search, page: String(page), ...(center ? { workCenterId: center } : {}), ...(status ? { status } : {}), ...(assignee?{assigneeId:assignee}:{}), ...(priorityFilter?{priority:priorityFilter}:{}), ...(deadline?{deadline}:{}) });
