...
function TaskCard({ task, onAction, onOpen, now, labels, canCancel = false }: { task: Operation; onAction: (task: Operation, action: string) => void; onOpen: () => void; now: number; labels:Record<string,string>; canCancel?: boolean }) {
  ...
  return <article className={`kanban-task ${task.status.toLowerCase()} work-center--${workCenterTone[task.workCenter.name]||"default"}`}>
    ...
    <div className="task-actions">
      {task.status === "QUEUED" && <button className={blocked?"needs-confirmation":""} onClick={() => onAction(task, "start")}><Play size={14}/>{blocked?"Подтвердить запуск":"Начать"}</button>}
...
export function OperationsScreen({ planner = false, initialCenter = "", focusId = "", initialStatus = "", onOpenChat }: { planner?: boolean; initialCenter?: string; focusId?: string; initialStatus?:string; onOpenChat?:(orderId:string)=>void }) {
  ...
  const [reason, setReason] = useState(""), [comment, setComment] = useState(""), [dependencyOverride,setDependencyOverride]=useState<"PREVIOUS_NOT_STARTED"|"PLANNING_ERROR"|"">(""), [now, setNow] = useState(Date.now());
  ...
  useEffect(() => { if(planner) api<(Assignee&{active:boolean;role:string})[]>("/staff").then(rows=>setAssignees(rows.filter(row=>row.active&&row.role==="EMPLOYEE"))).catch(error=>setError(error.message)); }, [planner]);
  ...
  function action(task: Operation, action: string) { setModal({ task, action }); setReason(""); setComment(""); setDependencyOverride(""); setError(""); }
  async function submit() {
    if (!modal || saving) return;
    const needsConfirmation=modal.action==="start"&&modal.task.predecessors.some(step=>step.status!=="COMPLETED");
    if(needsConfirmation&&!dependencyOverride){setError("Подтвердите причину запуска без отметки предыдущего участка");return;}
    setSaving(true); setError("");
    try { const handoff=modal.action==="complete"?nextHandoff(modal.task):[]; await api(`/operations/${modal.task.id}/actions`, { method: "POST", body: JSON.stringify({ action: modal.action, reason, comment, ...(dependencyOverride?{dependencyOverride}:{}) }) }); setModal(null); if(handoff.length)setHandoff({itemName:modal.task.itemName,centers:handoff}); setRefresh(value => value + 1); }
  ...
  return <div className="content production-board">
    ...
    {!planner&&<label className="employee-order-search"><Search size={17}/><input aria-label="Поиск заказа" placeholder="Номер заказа или производства" value={search} onChange={event=>{setSearch(event.target.value);setPage(1);}}/></label>}
    ...
    {planner&&<div className="view-switch" role="group" aria-label="Представление задач"><button className={view==="board"?"selected":"secondary"} onClick={()=>setView("board")}>Доска</button><button className={view==="timeline"?"selected":"secondary"} onClick={()=>setView("timeline")}>График</button></div>}
    ...
    {modal && <div className="modal-backdrop action-backdrop"><form className="task-dialog" role="dialog" aria-modal="true" aria-label={actionLabels[modal.action]} onSubmit={event => { event.preventDefault(); submit(); }}><header><h2>{actionLabels[modal.action]}</h2><button type="button" aria-label="Закрыть действие" disabled={saving} onClick={() => setModal(null)}><X/></button></header><p>{modal.task.itemName} · {modal.task.quantity} шт.</p>{modal.action==="start"&&modal.task.predecessors.some(step=>step.status!=="COMPLETED")&&<section className="dependency-override"><small>ТРЕБУЕТСЯ ПОДТВЕРЖДЕНИЕ</small><h3>Подтвердите поступление заказа</h3><p>Предыдущий участок{modal.task.predecessors.filter(step=>step.status!=="COMPLETED").length>1?"и":""}: <b>{[...new Set(modal.task.predecessors.filter(step=>step.status!=="COMPLETED").map(step=>step.workCenter.name))].join(", ")}</b> — ещё не отметил{modal.task.predecessors.filter(step=>step.status!=="COMPLETED").length>1?"и":""} работу в системе. Подтвердите, что изделие фактически поступило к вам и вы принимаете его в работу.</p><label><input required type="radio" name="dependencyOverride" checked={dependencyOverride==="PREVIOUS_NOT_STARTED"} onChange={()=>setDependencyOverride("PREVIOUS_NOT_STARTED")}/>Предыдущий участок не отметил запуск в системе</label><label><input required type="radio" name="dependencyOverride" checked={dependencyOverride==="PLANNING_ERROR"} onChange={()=>setDependencyOverride("PLANNING_ERROR")}/>Ошибка планирования маршрута</label></section>}{["pause", "cancel"].includes(modal.action) && <label>{modal.action === "cancel" ? "Причина отмены" : "Причина остановки"}{stopReasons.length?<select autoFocus required value={reason} onChange={event=>setReason(event.target.value)}><option value="">Выберите причину</option>{stopReasons.map(item=><option key={item.id} value={item.name}>{item.name}</option>)}<option value="Другая причина">Другая причина</option></select>:<input autoFocus required maxLength={1000} value={reason} onChange={event => setReason(event.target.value)}/>}</label>}{reason==="Другая причина"&&<label>Укажите причину<input required maxLength={1000} onChange={event=>setReason(event.target.value)}/></label>}{["pause", "comment", "problem"].includes(modal.action) && <label>Комментарий<textarea required={modal.action !== "pause"} maxLength={2000} value={comment} onChange={event => setComment(event.target.value)}/></label>}{error && <div role="alert" className="planning-error">{error}</div>}<footer><button type="button" className="secondary" disabled={saving} onClick={() => setModal(null)}>Отмена</button><button className="primary" disabled={saving}>{saving ? "Сохраняю…" : modal.action==="start"&&modal.task.predecessors.some(step=>step.status!=="COMPLETED")?"Подтвердить и начать":actionLabels[modal.action]}</button></footer></form></div>}
