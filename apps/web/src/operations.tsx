...
export function OperationsScreen({ planner = false, initialCenter = "", focusId = "", initialStatus = "" }: { planner?: boolean; initialCenter?: string; focusId?: string; initialStatus?:string }) {
  ...
  return <div className="content production-board">
    ...
    {loading ? <p>Загрузка задач…</p> : view === "timeline" && planner ? <Timeline tasks={data.items} now={now} labels={labels} colors={colors} onOpen={task=>setOpened(task)}/> : <div className="kanban">{Object.entries(labels).map(([key, label]) => {const rows=positions.filter(group=>positionStatus(group.tasks)===key);return <section className={`kanban-column ${key.toLowerCase()}`} style={{borderTopColor:colors[key]}} key={key}><h3>{label}<span>{rows.length}</span></h3>{rows.map(group=><PositionCard key={group.id} group={group} labels={labels} colors={colors} onOpen={setOpened}/>)}{!rows.length && <p className="column-empty">Нет позиций</p>}</section>;})}</div>}
