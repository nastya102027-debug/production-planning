...
const operationInclude = {
  ...
  launchItem: { include: { launch: { select: { number: true, plannedStart: true } }, route: { include: routeInclude }, operations: { include: { workCenter: true, predecessors: true } }, orderItem: { select: { id: true, name: true, quantity: true, comment: true, order: { select: { id: true, productionOrderNumber: true } } } } } },
...
export function productionRouter(prisma: PrismaClient) {
  ...
  router.get("/route-templates",plannerOnly,async(req,res)=>{const query=z.object({search:z.string().trim().max(100).default(""),archived:z.enum(["true","false"]).optional()}).parse(req.query);res.json(await prisma.routeTemplate.findMany({where:{archivedAt:query.archived==="true"?{not:null}:null,...(query.search?{OR:[{name:{contains:query.search,mode:"insensitive"}},{description:{contains:query.search,mode:"insensitive"}},{category:{contains:query.search,mode:"insensitive"}}]}:{})},include:templateInclude,orderBy:{updatedAt:"desc"}}));});
  ...
  router.put("/route-templates/:id",plannerOnly,async(req,res)=>{const id=z.string().uuid().parse(req.params.id),input=templateInput(req.body);const template=await prisma.$transaction(async tx=>{const current=await tx.routeTemplate.findFirst({where:{id,archivedAt:null}});if(!current)throw new ProductionError(404,"Шаблон не найден или архивирован");const ids=[...new Set(input.steps.map(step=>step.workCenterId))];if(await tx.workCenter.count({where:{id:{in:ids},active:true}})!==ids.length)throw new ProductionError(400,"Участок не найден или отключён");await tx.routeTemplateStep.deleteMany({where:{templateId:id}});await tx.routeTemplate.update({where:{id},data:{name:input.name,description:input.description||null,category:input.category||null}});await saveTemplateSteps(tx,id,input.steps);await tx.auditLog.create({data:{actorId:req.session!.sub,action:"ROUTE_TEMPLATE_UPDATED",entityType:"RouteTemplate",entityId:id,after:{name:input.name,steps:input.steps.length}}});return tx.routeTemplate.findUniqueOrThrow({where:{id},include:templateInclude});});res.json(template);});
  router.patch("/route-templates/:id",plannerOnly,async(req,res)=>{const id=z.string().uuid().parse(req.params.id),input=z.object({name:z.string().trim().min(1).max(200),description:z.string().trim().max(2000).nullable(),category:z.string().trim().max(120).nullable()}).parse(req.body);const template=await prisma.routeTemplate.update({where:{id},data:input}).catch(()=>null);if(!template)throw new ProductionError(404,"Шаблон не найден");await prisma.auditLog.create({data:{actorId:req.session!.sub,action:"ROUTE_TEMPLATE_METADATA_UPDATED",entityType:"RouteTemplate",entityId:id,after:input}});res.json(await prisma.routeTemplate.findUniqueOrThrow({where:{id},include:templateInclude}));});
  router.post("/route-templates/:id/copy",plannerOnly,async(req,res)=>{const id=z.string().uuid().parse(req.params.id),name=z.object({name:z.string().trim().min(1).max(200).optional()}).parse(req.body).name;const copy=await prisma.$transaction(async tx=>{const source=await tx.routeTemplate.findUnique({where:{id},include:templateInclude});if(!source)throw new ProductionError(404,"Шаблон не найден");const created=await tx.routeTemplate.create({data:{name:name??`${source.name} — копия`,description:source.description,category:source.category,authorId:req.session!.sub}});const byId=new Map<string,string>();for(const step of source.steps){const row=await tx.routeTemplateStep.create({data:{templateId:created.id,workCenterId:step.workCenterId,title:step.title,position:step.position,canvasX:step.canvasX,canvasY:step.canvasY,material:step.material,quantity:step.quantity,unit:step.unit,comment:step.comment,components:step.components===null?Prisma.JsonNull:step.components as Prisma.InputJsonValue}});byId.set(step.id,row.id);}for(const step of source.steps)for(const link of step.predecessors)await tx.routeTemplateStepDependency.create({data:{predecessorId:byId.get(link.predecessorId)!,successorId:byId.get(step.id)!}});await tx.auditLog.create({data:{actorId:req.session!.sub,action:"ROUTE_TEMPLATE_COPIED",entityType:"RouteTemplate",entityId:created.id,after:{sourceId:id}}});return tx.routeTemplate.findUniqueOrThrow({where:{id:created.id},include:templateInclude});});res.status(201).json(copy);});
  router.patch("/route-templates/:id/archive",plannerOnly,async(req,res)=>{const id=z.string().uuid().parse(req.params.id),archived=z.object({archived:z.boolean()}).parse(req.body).archived;const template=await prisma.routeTemplate.update({where:{id},data:{archivedAt:archived?new Date():null}}).catch(()=>null);if(!template)throw new ProductionError(404,"Шаблон не найден");await prisma.auditLog.create({data:{actorId:req.session!.sub,action:archived?"ROUTE_TEMPLATE_ARCHIVED":"ROUTE_TEMPLATE_RESTORED",entityType:"RouteTemplate",entityId:id,after:{archived}}});res.json(template);});
  router.delete("/route-templates/:id",plannerOnly,async(req,res)=>{const id=z.string().uuid().parse(req.params.id);const template=await prisma.routeTemplate.findUnique({where:{id},include:{_count:{select:{appliedRoutes:true}}}});if(!template)throw new ProductionError(404,"Шаблон не найден");if(template._count.appliedRoutes){await prisma.routeTemplate.update({where:{id},data:{archivedAt:new Date()}});await prisma.auditLog.create({data:{actorId:req.session!.sub,action:"ROUTE_TEMPLATE_ARCHIVED_ON_DELETE",entityType:"RouteTemplate",entityId:id}});return res.status(200).json({archived:true});}await prisma.routeTemplate.delete({where:{id}});await prisma.auditLog.create({data:{actorId:req.session!.sub,action:"ROUTE_TEMPLATE_DELETED",entityType:"RouteTemplate",entityId:id}});res.status(204).end();});
  router.post("/order-items/:itemId/route-templates/:templateId/apply",plannerOnly,async(req,res)=>{const itemId=z.string().uuid().parse(req.params.itemId),templateId=z.string().uuid().parse(req.params.templateId);const route=await prisma.$transaction(async tx=>{const [item,template]=await Promise.all([tx.orderItem.findFirst({where:{id:itemId,archivedAt:null,order:{archivedAt:null}}}),tx.routeTemplate.findFirst({where:{id:templateId,archivedAt:null},include:{steps:{include:{predecessors:true},orderBy:{position:"asc"}}}})]);if(!item)throw new ProductionError(404,"Позиция не найдена");if(!template?.steps.length)throw new ProductionError(400,"Шаблон недоступен или не содержит этапов");const version=(await tx.route.aggregate({where:{orderItemId:itemId},_max:{version:true}}))._max.version??0;const created=await tx.route.create({data:{orderItemId:itemId,name:template.name,version:version+1,sourceTemplateId:template.id}});const byId=new Map<string,string>();for(const step of template.steps){const row=await tx.routeStep.create({data:{routeId:created.id,workCenterId:step.workCenterId,title:step.title,position:step.position,canvasX:step.canvasX,canvasY:step.canvasY,material:step.material,quantity:step.quantity,unit:step.unit,comment:step.comment,components:step.components===null?Prisma.JsonNull:step.components as Prisma.InputJsonValue}});byId.set(step.id,row.id);}for(const step of template.steps)for(const link of step.predecessors)await tx.routeStepDependency.create({data:{predecessorId:byId.get(link.predecessorId)!,successorId:byId.get(step.id)!}});await tx.auditLog.create({data:{actorId:req.session!.sub,action:"ROUTE_TEMPLATE_APPLIED",entityType:"Route",entityId:created.id,after:{templateId}}});return tx.route.findUniqueOrThrow({where:{id:created.id},include:routeInclude});});res.status(201).json(route);});
