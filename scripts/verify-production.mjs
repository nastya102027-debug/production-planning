...
try {
  ...
  const templateBody={name:"Зеркало поворотное",description:"Параллельная обработка зеркала",category:"Зеркала",nodes:[{id:"saw",workCenterId:centerA.id,title:"Пила",material:"Латунь 5 мм",quantity:1,unit:"шт.",components:[]},{id:"laser",workCenterId:centerB.id,title:"Лазер",material:"Латунь 2 мм",quantity:1,unit:"шт.",components:[]},{id:"weld",workCenterId:centerA.id,title:"Сварка",material:null,quantity:null,unit:"шт.",components:[]}],edges:[{source:"saw",target:"weld"},{source:"laser",target:"weld"}]};
  check((await request("/route-templates",workerCookie,templateBody)).status===403,"Employee cannot manage route templates");
  ...
  const copiedTemplate=await request(`/route-templates/${template.data.id}/copy`,cookie,{});
  check(copiedTemplate.status===201,"Planner can copy a route template");
